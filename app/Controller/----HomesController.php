...
class HomesController extends AppController {
  //{
	...
	var $uses = array('Admin','UserMaster','City','Locality','Category','Community','UserVerfication','ClassSegment','ClassType','ClassPost','VendorClasse','ClassSchedule');
	 var $components = array('Paginator','Messages','Session');

    public function checkUser(){
    	//print_r($this->Session->check('Login1'));
        //die;
...
public function savepostClass(){
 ...
 ###################### 2 ##################################
  ...
  /* Start Image Upload 1*/
            ...
            $target_dir = WWW_ROOT."img/"."ClasssImage/";
...
public function findsubCat(){
       ...
       $cat_sub=$this->ClassSegment->find('all',array('conditions'=>array('category_id'=>$cat_id))); 
         echo '<select class="form-control reg_input" name="segment_id" id="segment_id">
...
public function classSchedule(){
       ...
       $this->autoRender=false;
	   //print_r($_POST);
	  $data = $this->request->data['data_1'];
	  $time = $this->request->data['time_1'];
	  $this->request->data['session_date']=$data;
	  $this->request->data['session_time']=$time;
	  $this->request->data['add_date']= date('dd-mm-yy');
	  $this->request->data['modified_date']= date('dd-mm-yy');
	  $this->request->data['status']= 0;
	  $this->request->data['session_id']= 's_'.rand(10,100);
	  
	  
	  
	   if($this->ClassSchedule->save($this->request->data)){
		   
		   $cs_id=$this->ClassSchedule->getLastInsertId();
